      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            SizedBox(height: 8),
            Text('Suas Coleções', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
            SizedBox(height: 24),
            ...
            Expanded(
              child: _carregando
                ...
                : colecoes.isEmpty
                  ? Center(child: Text('Nenhuma coleção ainda', style: TextStyle(color: Colors.white54)))
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            title: Text(colecao['nameCollection'] ?? '', style: TextStyle(color: Colors.white)),
                            subtitle: Text('${(colecao['cards'] as List).length} cartas', style: TextStyle(color: Colors.white38, fontSize: 12)),
                            trailing: Icon(Icons.arrow_forward_ios, color: Colors.white38, size: 16),
                            onTap: () async {
                              // vai pra tela da coleção e recarrega ao voltar
                              await Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => Collection(colecao: colecao),
                                ),
                              );
                              _carregarColecoes(); // atualiza ao voltar
                            },
                          ),
                        );
                      },
            ...
            SizedBox(
              ...
              child: MyButton(
                'Nova Coleção',
                onPressed: () async {
                  await Navigator.push(context, MaterialPageRoute(builder: (context) => CreateCollection()));
                  _carregarColecoes(); // recarrega ao voltar
                },
              ),
            ),
